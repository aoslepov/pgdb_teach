...
-- * пересчёт цен будет происходить только по конкретной позиции товара
-- * отчёт будет подготовлен предварительно и будет выведен по требованию моментально
-- * доступы к отчёту можно гибко разграничить
-- * не требует разработки на уровне бекэнда
```
