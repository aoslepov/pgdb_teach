...
```
-- дедлок может возникнуть при изменении порядка обхода строк при блокировках разных транзакций
```
...
-- при попытке обновить все строки во второй сессии (573) необходима эксклюзивная блокировка всех строк, 
-- но блокировку держит первая сессия (564), транзакция страновится в очередь
...
-- при попытке обновить данные в первой сессии будет попытка эксклюзивной блокировки всех строк 
-- т.к. вторая сессия (573) уже ожидает блокировки, а есть строки, которые заблокированны первой сессией (564)
-- это будет невозможно и транзакция будет убита процессом поиска дедлоков
lock=*# update accounts set amount=10;
ERROR:  deadlock detected
